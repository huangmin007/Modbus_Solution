...
static uint16_t crc16(uint8_t address, uint8_t* pduFrame, uint16_t pduLen)
{
    uint8_t CRCHi = 0xFF, CRCLo = 0x0FF, Index;

    Index = CRCHi ^ address;
#if USE_PROGMEM_CRC
    CRCHi = CRCLo ^ pgm_read_byte_near(table_crc_hi + Index);
    CRCLo = pgm_read_byte_near(table_crc_lo + Index);
#else
    CRCHi = CRCLo ^ table_crc_hi[Index];
    CRCLo = table_crc_lo[Index];
#endif
